/**
 * @api {JANELA} Assembleias Gerir dados
 * @apiName eleicoesSegurasAssembleias
 * @apiGroup Assembleias
 * 
 * @apiDescription Gestão de Assembleias:
 * - Listar Assembleias
 * - Inserir Assembleia
...
export default class Assembleias extends React.Component{ //Begin Component (class) Assembleias for eleicoesSeguras Application
	constructor(props){ //Begin constructor for Assembleias
		...
		this.state = { //Begin state of the current component to save all internal variables
			DBTable: 'Assembleias', //DB Table for this component
			DBRoute: 'Assembleias', //DB Backend route for this component
			...
			dataGet: 'devolverAssembleias', //Socket message to handle gotten data. 
	...
	} //End constructor for Assembleias
		...
		return ( //Returning the component wraper with everithing inside it
			<div> {/* Begin binding*/}
				<Template //Begin Generic Template
					component={ //Begin current (specific = Assembleias) component -> The only prop needed to be passed, it contains all the JSX for the current component (Assembleias)
						<div className="col row"> {/* Begin Main Content single full row*/}
							<div className="col-lg-12 col-md-12 col-sm-12">{/* Begin Main Content single col*/}
								<h1 className="pageTitle">{Languages.languages[this.state.language].Assembleias.name}</h1>{/*Current Component (Assembleias) main title*/}
								<Tabs> {/* Begin Tabs for header Subtitles in the current Component (Assembleias) */}
									<TabList> {/* Bengin enclosing for different tab section. TODO: make this dynamic according to number of different types of Assembleias */}
										{//Begin Javascript section to use dynamic content (tabs) using map()
											Languages.languages[this.state.language].Assembleias.mainTabs.map((tab, index)=>(
									...
									</TabList> {/* End enclosing for different tab section. TODO: make this dynamic according to number of different types of Assembleias */}
									{//Begin Javascript section to use dynamic content (tabs content) using map
										Languages.languages[this.state.language].Assembleias.mainTabs.map((tab, index)=>( //TODO: Use this tab variable in the filter process someway
											<TabPanel key={index}> {/* Bengin content for the n-th Tab inside TabList */}
												<div className="row"> {/* Begin Content row to enclose the dynamic table*/}
													...
													<DynamicTable 
														...
														saveButton = {Languages.languages[this.state.language].Assembleias.saveButton} //Button Save Label
														editButton = {Languages.languages[this.state.language].Assembleias.editButton} //Button Edit Label
														deleteButton = {Languages.languages[this.state.language].Assembleias.deleteButton} //Button Delete Label
														lockButton = {Languages.languages[this.state.language].Assembleias.lockButton} //Button Delete Label
														titleSave = {Languages.languages[this.state.language].Assembleias.titleSave} //Page Save tittle, coinciding with the Button save for now
														titleEdit = {Languages.languages[this.state.language].Assembleias.titleEdit} //Page Edit tittle
														titleDelete = {Languages.languages[this.state.language].Assembleias.titleDelete} //Page Delete tittle
														messageDelete = {Languages.languages[this.state.language].Assembleias.messageDelete} //Page Delete message
														successSave = {Languages.languages[this.state.language].Assembleias.successSave} //Success POST message
														successEdit = {Languages.languages[this.state.language].Assembleias.successEdit} //Success PUP message
														successDelete = {Languages.languages[this.state.language].Assembleias.successDelete} //Success DELETE message
														...
														errorSaveClient = {Languages.languages[this.state.language].Assembleias.errorSaveClient} //Client Side Error message in case case of some exception during POST
														errorSaveServer = {Languages.languages[this.state.language].Assembleias.errorSaveServer} //Server Side Error message in case case of some exception during POST
														errorEditClient = {Languages.languages[this.state.language].Assembleias.errorEditClient} //Client Side  Error message in case case of some exception during PUT
														errorEditServer = {Languages.languages[this.state.language].Assembleias.errorEditServer} //Server Side  Error message in case case of some exception during PUT
														errorDeleteClient = {Languages.languages[this.state.language].Assembleias.errorDeleteClient} //Client Side  Error message in case case of some exception during DELETE
														errorDeleteServer = {Languages.languages[this.state.language].Assembleias.errorDeleteServer} //Server Side  Error message in case case of some exception during DELETE
														emptyMessage = {Languages.languages[this.state.language].Assembleias.emptyMessage} //Exception message for no data available
														saveButtonHint = {Languages.languages[this.state.language].Assembleias.saveButtonHint} //The hint to be upon save Button. TODO: not in use yet
														editButtonHint = {Languages.languages[this.state.language].Assembleias.editButtonHint} //The hint to be upon edit Button. 
														deleteButtonHint = {Languages.languages[this.state.language].Assembleias.deleteButtonHint} //The hint to be upon delete Button. 
														lockButtonHint = {Languages.languages[this.state.language].Assembleias.lockButtonHint} //The hint to be upon block Button. TODO: discuss  with Antonio and implement the real purpose of this button
														lockButtonMessage = {Languages.languages[this.state.language].Assembleias.lockButtonMessage} //The message to be chown by clicking block Button . TODO: discuss  with Antonio and implement the real purpose of this button
														...
														actionsTitle = {Languages.languages[this.state.language].Assembleias.actionsTitle} //Header actions where titleColor is
								...
								</Tabs> {/* End Tabs for header Subtitles in the current Component (Assembleias) */}
							</div> {/* end Main Content single col*/}	
						</div> /* Begin Main Content single full row*/	
					}/* End current (specific = Assembleias) component*/
				/> {/*End Generic Template*/}
			</div>/* End binding*/
		); //End returning the component wraper with everithing inside it
	} //End Binding method
} //Begin Component (class) Assembleias for eleicoesSeguras Application - 6292a9bb828f4cfbd5b16ba4a3c1228b6db7f188996a20c2c89998ea49ae6a48311add05e3b89b1f616afdeb7b32dc004cce351bdbd6ec9c6dcda57071e25513
