/**
 * @api {JANELA} Partidos Gerir dados
 * @apiName eleicoesSegurasPartidos
 * @apiGroup Partidos
 * 
 * @apiDescription Gestão de Partidos:
 * - Listar Partidos
 * - Inserir Partido
 * - Atualizar Partido
 * - Eliminar Partido
...
export default class Partidos extends React.Component{ //Begin Component (class) Partidos for eleicoesSeguras Application
	constructor(props){ //Begin constructor for Partidos
		...
		this.state = { //Begin state of the current component to save all internal variables
			DBTable: 'partidos', //DB Table for this component
			DBRoute: 'partidos', //DB Backend route for this component
			...
			dataGet: 'devolverPartidos', //Socket message to handle gotten data. 
		...
		return ( //Returning the component wraper with everithing inside it
			<div> {/* Begin binding*/}
				<Template //Begin Generic Template
					component={ //Begin current (specific = Partidos) component -> The only prop needed to be passed, it contains all the JSX for the current component (Partidos)
						<div className="col row"> {/* Begin Main Content single full row*/}
							<div className="col-lg-12 col-md-12 col-sm-12">{/* Begin Main Content single col*/}
								<h1 className="pageTitle">{Languages.languages[this.state.language].Partidos.name}</h1>{/*Current Component (Partidos) main title*/}
								<Tabs> {/* Begin Tabs for header Subtitles in the current Component (Partidos) */}
									<TabList> {/* Bengin enclosing for different tab section. TODO: make this dynamic according to number of different types of Partidos */}
										{//Begin Javascript section to use dynamic content (tabs) using map()
											Languages.languages[this.state.language].Partidos.mainTabs.map((tab, index)=>(
									...
									</TabList> {/* End enclosing for different tab section. TODO: make this dynamic according to number of different types of Partidos */}
									{//Begin Javascript section to use dynamic content (tabs content) using map
										Languages.languages[this.state.language].Partidos.mainTabs.map((tab, index)=>( //TODO: Use this tab variable in the filter process someway
											<TabPanel key={index}> {/* Bengin content for the n-th Tab inside TabList */}
												<div className="row"> {/* Begin Content row to enclose the dynamic table*/}
													...
													<DynamicTable 
														...
														saveButton = {Languages.languages[this.state.language].Partidos.saveButton} //Button Save Label
														editButton = {Languages.languages[this.state.language].Partidos.editButton} //Button Edit Label
														deleteButton = {Languages.languages[this.state.language].Partidos.deleteButton} //Button Delete Label
														lockButton = {Languages.languages[this.state.language].Partidos.lockButton} //Button Delete Label
														titleSave = {Languages.languages[this.state.language].Partidos.titleSave} //Page Save tittle, coinciding with the Button save for now
														titleEdit = {Languages.languages[this.state.language].Partidos.titleEdit} //Page Edit tittle
														titleDelete = {Languages.languages[this.state.language].Partidos.titleDelete} //Page Delete tittle
														messageDelete = {Languages.languages[this.state.language].Partidos.messageDelete} //Page Delete message
														successSave = {Languages.languages[this.state.language].Partidos.successSave} //Success POST message
														successEdit = {Languages.languages[this.state.language].Partidos.successEdit} //Success PUP message
														successDelete = {Languages.languages[this.state.language].Partidos.successDelete} //Success DELETE message
														...
														errorSaveClient = {Languages.languages[this.state.language].Partidos.errorSaveClient} //Client Side Error message in case case of some exception during POST
														errorSaveServer = {Languages.languages[this.state.language].Partidos.errorSaveServer} //Server Side Error message in case case of some exception during POST
														errorEditClient = {Languages.languages[this.state.language].Partidos.errorEditClient} //Client Side  Error message in case case of some exception during PUT
														errorEditServer = {Languages.languages[this.state.language].Partidos.errorEditServer} //Server Side  Error message in case case of some exception during PUT
														errorDeleteClient = {Languages.languages[this.state.language].Partidos.errorDeleteClient} //Client Side  Error message in case case of some exception during DELETE
														errorDeleteServer = {Languages.languages[this.state.language].Partidos.errorDeleteServer} //Server Side  Error message in case case of some exception during DELETE
														emptyMessage = {Languages.languages[this.state.language].Partidos.emptyMessage} //Exception message for no data available
														saveButtonHint = {Languages.languages[this.state.language].Partidos.saveButtonHint} //The hint to be upon save Button. TODO: not in use yet
														editButtonHint = {Languages.languages[this.state.language].Partidos.editButtonHint} //The hint to be upon edit Button. 
														deleteButtonHint = {Languages.languages[this.state.language].Partidos.deleteButtonHint} //The hint to be upon delete Button. 
														lockButtonHint = {Languages.languages[this.state.language].Partidos.lockButtonHint} //The hint to be upon block Button. TODO: discuss  with Antonio and implement the real purpose of this button
														lockButtonMessage = {Languages.languages[this.state.language].Partidos.lockButtonMessage} //The message to be chown by clicking block Button . TODO: discuss  with Antonio and implement the real purpose of this button
														...
														actionsTitle = {Languages.languages[this.state.language].Partidos.actionsTitle} //Header actions where titleColor is
								...
								</Tabs> {/* End Tabs for header Subtitles in the current Component (Partidos) */}
							</div> {/* end Main Content single col*/}	
						</div> /* Begin Main Content single full row*/	
					}/* End current (specific = Partidos) component*/
				/> {/*End Generic Template*/}
			</div>/* End binding*/
		); //End returning the component wraper with everithing inside it
	} //End Binding method
} //Begin Component (class) Partidos for eleicoesSeguras Application - 6292a9bb828f4cfbd5b16ba4a3c1228b6db7f188996a20c2c89998ea49ae6a48311add05e3b89b1f616afdeb7b32dc004cce351bdbd6ec9c6dcda57071e25513
