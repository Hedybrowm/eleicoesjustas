/**
 * @api {JANELA} Votos Gerir dados
 * @apiName eleicoesSegurasVotos
 * @apiGroup Votos
 * 
 * @apiDescription Gestão de Votos:
 * - Listar Votos
 * - Inserir Voto
 * - Atualizar Voto
 * - Eliminar Voto
...
export default class Votos extends React.Component{ //Begin Component (class) Votos for eleicoesSeguras Application
	constructor(props){ //Begin constructor for Votos
		...
		this.state = { //Begin state of the current component to save all internal variables
			DBTable: 'votos', //DB Table for this component
			DBRoute: 'votos', //DB Backend route for this component
			...
			dataGet: 'devolverVotos', //Socket message to handle gotten data. 
		...
		return ( //Returning the component wraper with everithing inside it
			<div> {/* Begin binding*/}
				<Template //Begin Generic Template
					component={ //Begin current (specific = Votos) component -> The only prop needed to be passed, it contains all the JSX for the current component (Votos)
						<div className="col row"> {/* Begin Main Content single full row*/}
							<div className="col-lg-12 col-md-12 col-sm-12">{/* Begin Main Content single col*/}
								...
								<DynamicTable
									...
									saveButton = {Languages.languages[this.state.language].Votos.saveButton} //Button Save Label
									editButton = {Languages.languages[this.state.language].Votos.editButton} //Button Edit Label
									deleteButton = {Languages.languages[this.state.language].Votos.deleteButton} //Button Delete Label
									lockButton = {Languages.languages[this.state.language].Votos.lockButton} //Button Delete Label
									titleSave = {Languages.languages[this.state.language].Votos.titleSave} //Page Save tittle, coinciding with the Button save for now
									titleEdit = {Languages.languages[this.state.language].Votos.titleEdit} //Page Edit tittle
									titleDelete = {Languages.languages[this.state.language].Votos.titleDelete} //Page Delete tittle
									messageDelete = {Languages.languages[this.state.language].Votos.messageDelete} //Page Delete message
									successSave = {Languages.languages[this.state.language].Votos.successSave} //Success POST message
									successEdit = {Languages.languages[this.state.language].Votos.successEdit} //Success PUP message
									successDelete = {Languages.languages[this.state.language].Votos.successDelete} //Success DELETE message
									...
									errorSaveClient = {Languages.languages[this.state.language].Votos.errorSaveClient} //Client Side Error message in case case of some exception during POST
									errorSaveServer = {Languages.languages[this.state.language].Votos.errorSaveServer} //Server Side Error message in case case of some exception during POST
									errorEditClient = {Languages.languages[this.state.language].Votos.errorEditClient} //Client Side  Error message in case case of some exception during PUT
									errorEditServer = {Languages.languages[this.state.language].Votos.errorEditServer} //Server Side  Error message in case case of some exception during PUT
									errorDeleteClient = {Languages.languages[this.state.language].Votos.errorDeleteClient} //Client Side  Error message in case case of some exception during DELETE
									errorDeleteServer = {Languages.languages[this.state.language].Votos.errorDeleteServer} //Server Side  Error message in case case of some exception during DELETE
									emptyMessage = {Languages.languages[this.state.language].Votos.emptyMessage} //Exception message for no data available
									saveButtonHint = {Languages.languages[this.state.language].Votos.saveButtonHint} //The hint to be upon save Button. TODO: not in use yet
									editButtonHint = {Languages.languages[this.state.language].Votos.editButtonHint} //The hint to be upon edit Button. 
									deleteButtonHint = {Languages.languages[this.state.language].Votos.deleteButtonHint} //The hint to be upon delete Button. 
									lockButtonHint = {Languages.languages[this.state.language].Votos.lockButtonHint} //The hint to be upon block Button. TODO: discuss  with Antonio and implement the real purpose of this button
									lockButtonMessage = {Languages.languages[this.state.language].Votos.lockButtonMessage} //The message to be chown by clicking block Button . TODO: discuss  with Antonio and implement the real purpose of this button
									...
									actionsTitle = {Languages.languages[this.state.language].Votos.actionsTitle} //Header actions where titleColor is
					...
					}/* End current (specific = Votos) component*/
				/> {/*End Generic Template*/}
			</div>/* End binding*/
		); //End returning the component wraper with everithing inside it
	} //End Binding method
} //Begin Component (class) Votos for eleicoesSeguras Application - 6292a9bb828f4cfbd5b16ba4a3c1228b6db7f188996a20c2c89998ea49ae6a48311add05e3b89b1f616afdeb7b32dc004cce351bdbd6ec9c6dcda57071e25513
